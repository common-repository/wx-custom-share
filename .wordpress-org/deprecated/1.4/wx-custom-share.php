...
function wxcs_get_access_token(){
	...
	$ws_settings = get_option('ws_settings');
	$api_url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='. $ws_settings['ws_appid'] .'&secret='. $ws_settings['ws_appsecrect'];
...
function wxcs_api_config_notice(){
	...
	$ws_settings = get_option('ws_settings');
	if( ($ws_settings['ws_appid'] == '' || $ws_settings['ws_appsecrect'] == '') &&
		(
...
?>
		<div class="notice notice-warning">
			...
			<?php
			printf(
				__( 'If you want to share link in Wechat directly, please <strong>enter AppID and AppSecrect</strong>, and <strong>add <code>%s</code> to JSAPI Secure Domain</strong> on WeChat Admin Platform.', 'wx-custom-share' ),
				$_SERVER['HTTP_HOST']
			)
			?>
			<br>
...
?>
    <div class="wrap">
        ...
        <form method="post" action="options.php">
            ...
            <tr><th><label for="ws_settings[ws_appsecrect]"><?php _e('Wechat AppSecrect','wx-custom-share') ?></label></th>
			<td>
                <p>
                	<input type="text" id="ws_settings[ws_appsecrect]" name="ws_settings[ws_appsecrect]" value="<?php echo $ws_settings['ws_appsecrect'] ?>" class="regular-text" autocomplete="off">
                </p>
            </td></tr>
...
function wxcs_show_box() {
	...
	$default_url = $default_desc;
	$default_img = get_the_post_thumbnail_url($post);
	
	$meta_title = get_post_meta( $post->ID, 'ws_title', true );
	$meta_desc = get_post_meta( $post->ID, 'ws_desc', true );
	$meta_link_url = get_post_meta( $post->ID, 'ws_link_url', true );
	$meta_img = get_post_meta( $post->ID, 'ws_url', true );
?>
    ...
    <table class="form-table">
        <tr>
            ...
            <td>
            	<input type="text" name="ws-title" id="ws-title" value="<?php echo $meta_title ?>" style="width:80%" autocomplete="off" placeholder="<?php echo $default_title ?>">
        ...
        <tr>
            ...
            <td>
            	<input type="text" name="ws-desc" id="ws-desc" value="<?php echo $meta_desc ?>" style="width:80%" autocomplete="off" placeholder="<?php echo $default_desc ?>">
                <p class="description" style="font-size:12px">
        ...
        <tr>
        	<th style="width:20%"><label for="ws-link-url"><?php _e('Link URL','wx-custom-share') ?></label></th>
            <td>
            	<input type="text" name="ws-link-url" id="ws-link-url" value="<?php echo esc_url($meta_link_url) ?>" style="width:80%" autocomplete="off" placeholder="<?php echo $default_url ?>">
                <p class="description" style="font-size:12px">
        ...
        <tr>
        	<th style="width:20%"><label for="ws-url"><?php _e('Icon','wx-custom-share') ?></label></th>
            <td>
            	<input type="text" name="ws-url" id="ws-url" value="<?php echo esc_url( $meta_img ) ?>" style="width:80%" autocomplete="off" placeholder="<?php echo $default_img ?>">
                <button type="button" id="ws_upload_btn" class="button insert-media add_media"><?php _e('Media','wx-custom-share') ?></button>
                <p class="description" style="font-size:12px">
                ...
                <div class="ws-preview">
                	...
                	<div class="ws-timeline clearfix">
                        ...
                        <div class="ws-timeline-right">
                            ...
                            <a class="ws-link-url" href="<?php echo $meta_link_url ? $meta_link_url : $default_url ?>" target="_blank">
                				<div class="ws-timeline-link"><table><tr>
                    			<td style="position:relative">
									<img class="ws-img" src="" alt="<?php _e('cannot get the image','wx-custom-share') ?>">
									<div class="ws-img-mask" style="background-image:url(<?php echo $meta_img ? esc_url($meta_img) : ($default_img ? $default_img : $pngdata) ?>)"></div>
								</td>
                    			<td><div class="ws-timeline-title-div">
                        			<span class="ws-title ws-timeline-title"><?php echo $meta_title ? $meta_title : $default_title ?></span>
                        		</div></td>
                    			</tr></table></div>
                            </a>
                    ...
                    <div class="ws-chat clearfix">
                    	...
                    	<div class="ws-chat-main">
                        	<p class="ws-title ws-chat-title"><?php echo $meta_title ? $meta_title : $default_title ?></p>
                            <div class="ws-chat-desc-div">
                            	<div class="ws-chat-desc"><?php echo $meta_desc ? $meta_desc : $default_desc ?></div>
                            	<img class="ws-img" src="" alt="<?php _e('cannot get the image','wx-custom-share') ?>" style="float:right;">
								<div class="ws-img-mask" style="background-image:url(<?php echo $meta_img ? esc_url($meta_img) : ($default_img ? $default_img : $pngdata) ?>)"></div>
                            </div>
                        </div>
                        </div>
...
<script>
	jQuery('#ws_upload_btn').click(function(){
		...
		wp.media.editor.send.attachment = function(props, attachment){
			jQuery('.ws-img-mask').css('background-image', 'url('+ attachment.url + ')');
	...
	function syncImg(imgmask){
		jQuery(imgmask).css('background-image', 'url('+ jQuery(imgmask).attr('src') +')');
	}
	
	var df_title = '<?php echo $default_title ?>',
	    df_desc = '<?php echo $default_desc ?>';
		df_url = '<?php echo $default_url ?>',
		df_img = '<?php echo $default_img ? $default_img : $pngdata ?>';
	...
	wxcsBindValue('#ws-link-url', '.ws-link-url', 'attr', 'href', df_url);
	wxcsBindValue('#ws-url', '.ws-img-mask', 'attr', 'src', df_img);
</script>
<?php
}
...
function wxcs_add_share_info(){
	...
	if( is_singular() && '' !== $ws_settings['ws_display_types'] && array_key_exists( get_post_type(), $ws_settings['ws_display_types'] ) ){
		...
		if( $ws_settings['ws_appid'] != '' && $ws_settings['ws_appsecrect'] != '' && ($jsapi_ticket = wxcs_get_jsapi_ticket()) !== false ){
			$noncestr = wxcs_generate_noncestr();
			$timestamp = time();
			$url = wxcs_get_signature_url();
			$signature = wxcs_generate_signature($jsapi_ticket, $noncestr, $timestamp, $url);
		}
?>
		<script id="wx-custom-share-script">
		setShareInfo({
			title: '<?php echo $meta_title ? $meta_title : $default_title ?>',
			summary: '<?php echo $meta_desc ? $meta_desc : $default_desc ?>',
			pic: '<?php echo $meta_img ? $meta_img : $default_img ?>',
			url: '<?php echo $meta_link_url ? $meta_link_url : $default_url ?>',
<?php
			if( $ws_settings['ws_appid'] != '' && $ws_settings['ws_appsecrect'] != '' && $jsapi_ticket !== false ){				
?>
			WXconfig:{
				swapTitleInWX: <?php echo apply_filters( 'wxcs_wechat_timeline_swap_title', 'false' ) ?>,
