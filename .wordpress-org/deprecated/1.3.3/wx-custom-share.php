...
$wxcs_img_url = admin_url( 'admin-ajax.php' ) . '?action=resizeimg&s=';
...
function wxcs_add_action_links( $links ) {
	$mylinks = array(
		'<a href="' . admin_url('options-general.php?page=ws-settings') . '">'. __('Settings','wx-custom-share'). '</a>'
	);
...
function wxcs_menu(){
	...
	add_options_page( __('Wechat Share Settings','wx-custom-share'), __('Wechat Share','wx-custom-share'), 'administrator', 'ws-settings', 'wxcs_html_page' );
}
...
function wxcs_show_box() {
    global $wxcs_img_url, $ws_meta_box, $post;
...
?>
    ...
    <table class="form-table">
        <tr>
            ...
            <td>
            	<input type="text" name="ws-url" id="ws-url" value="<?php echo $meta ? esc_url( $meta ) : '' ?>" size="30" style="width:80%" autocomplete="off">
                ...
                <p class="description" style="font-size:12px">
					<span style="color:#ED143D;font-weight:bold"><?php _e('Custom icon in Wechat directly is invalid now because Wechat is Updated, you must open the link in QQ, and then share to Wechat friend or timeline.','wx-custom-share') ?></span>
					<br>
                	<?php _e('Enter a URL, Upload or choose from media library. And the image will be resized to 300*300. Please make sure the image you selected is square.','wx-custom-share') ?>
                    <br>
                    <?php _e('Support png, jpg, gif and wbmp','wx-custom-share') ?>
                </p>
            </td>
            <td></td>
        </tr>
                	...
                	<div class="ws-timeline clearfix">
                        ...
                        <div class="ws-timeline-right">
                            ...
                            <a href="<?php echo get_permalink($post) ?>" target="_blank">
                    			...
                    			<td><img class="ws-link-img" src="<?php echo $meta ? $wxcs_img_url . urlencode( esc_url($meta)) : $pngdata ?>" alt="<?php _e('cannot get the image','wx-custom-share') ?>"></td>
                    ...
                    <div class="ws-chat clearfix">
                    	...
                    	<div class="ws-chat-main">
                            ...
                            <div class="ws-chat-desc-div">
                            	...
                            	<img class="ws-link-img" src="<?php echo $meta ? $wxcs_img_url . urlencode( esc_url($meta) ) : $pngdata ?>" alt="<?php _e('cannot get the image','wx-custom-share') ?>" style="float:right;">
                            </div>
                        </div>
                        </div>
						</a>
...
function wxcs_save_data($post_id){
	...
	$new = esc_url( sanitize_text_field($_POST['ws-url']) );
	if( $new && $new != $old ){
...
function wxcs_show(){
	...
	if( is_singular() ):
		...
		if( '' !== $ws_settings['ws_display_types'] && array_key_exists( get_post_type(), $ws_settings['ws_display_types'] ) ):
			...
			if( $meta ):
				$ws_url = $wxcs_img_url . urlencode( esc_url($meta) );
?>
